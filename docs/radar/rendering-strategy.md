...
| ---------------------------------- | --------- | --------------------- | ------------------------------------------------------------------------------ |
| `/radar`                           | ISR       | Path                  | A successful issue publication                                                 |
...
Every ISR route also has a 24-hour time-based revalidation interval. This is a
recovery backstop, not the normal freshness mechanism. Exact on-demand
invalidation after a database write is the normal path, and the workflow must
verify the affected public pages before reporting success.
...
Radar does not query Neon from `generateStaticParams`. Parameterized issue and
article routes generate and enter the cache on their first request. This keeps a
deployment independent of database availability and avoids an arbitrary recent
window that would still fail to enumerate articles published between deploys.
...
- Adding a deep dive invalidates the article's permanent path and every dated
  ...
  appearances is the latest issue.
